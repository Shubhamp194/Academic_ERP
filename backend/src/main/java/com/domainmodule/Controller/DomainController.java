...
//        System.out.println(doms);
    ...
    public Response get_domain_byId(@PathParam("d_id") int id){
...
//        emp.setDepartment(emp.getDepartment());
    ...
    @POST
    @Path("/modify/{d_id}")
    public Response update_domain(@PathParam("d_id") int dom_id, Domain domObj){
        if(domDAO.modifyDomain(dom_id, domObj)){
            return Response.status(200).entity("Success").build();
        }
        return Response.status(404).entity("Failure while updating domain").build();
    }
